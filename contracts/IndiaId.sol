...
contract IndiaId is Country{
    ...
    function addPersonFromAuction(
        ...
        bytes8 _originCountry) public onlyOrg(msg.sender) returns(uint256){
            return pLib.addPerson(uTokenId, _name, _value, _originCountry, "India", this, owner());
        }
    function addPerson(
        bytes8 _name,
    ...
    event personTransfered(uint256, bool, bool, uint256);
    function getDetails(uint256 tokenId, uint256 value) public  onlyOrg(msg.sender) returns(bytes8, bytes8){
        ...
        emit personTransfered(tokenId, pLib.auctionStatus(tokenId), pLib.identityStatus(tokenId), pLib.getPopulation());
