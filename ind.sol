...
// File: contracts/SimpleAuction.sol
...
contract SimpleAuction {
    ...
    event UnsuccessfulAuction(address, uint256);
    ...
    constructor(uint256 _bid) public {
    ...
    function auctionEnd() public payable {
        ...
        // 3. Interaction
        if(highestBid > iniValue){
            // Bid has to be atleast 1 more than initial value for successful transaction.
                beneficiary.transfer(highestBid);
        }
        else{
            emit UnsuccessfulAuction(highestBidder, highestBid);
        }
    }
...
contract Organization is Ownable{
    ...
    function endAuction(
        ...
        if(countryOwners[highestBidder] != 0x0){
            //transfer ownership
            addingPersonFromAuction(uTokenId, lTokenId, highestBid, highestBidder);
    ...
    function addingPersonFromAuction(uint256 uTokenId, uint256 lTokenId, uint256 value, address highestBidder) internal{
        (bytes8 name,
            bytes8 originCountry) = countryNames[countryOwners[highestBidder]].getDetails(lTokenId, value);
...
contract IndiaId is Country{
    ...
    constructor(Organization _orgAddr) public{
        iniOrgAddr = _orgAddr;
        pLib = new PersonLib();
        iniOrgAddr.registerCountry(this, owner(), "India");
    }
    ...
    function addPersonFromAuction(
        ...
        bytes8 _originCountry) public onlyOrg(msg.sender) returns(uint256){
            pLib.addPerson(uTokenId, _name, _value, _originCountry, "India", this, owner());
        }
    function addPerson(
        bytes8 _name,
        bytes8 _originCountry
    )  public onlyOwner returns(uint256){
        uint256 uTokenId = iniOrgAddr.createToken(this, owner(), "India");
        //tk = uTokenId;
        uint tokenId = pLib.addPerson(uTokenId, _name, 10000, _originCountry, "India", this, owner());
        return tokenId;
    }
    ...
    function getIntoAuction(uint256 tokenId) public onlyOwner{
        ...
        iniOrgAddr.getIntoAuction(pLib.getUTokenID(tokenId), pLib.getValue(tokenId), owner(), "India");
    }
    ...
    function endAuction(uint256 tokenId) public inAuction(tokenId) checkStatus(tokenId){
        ...
        tk = uTI;
        iniOrgAddr.endAuction(uTI, tokenId, owner(), "India");
    }
